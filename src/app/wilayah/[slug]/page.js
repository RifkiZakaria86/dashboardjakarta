...
export default function WilayahPage() {
  ...
  if (!region) {
    return (
      <div className="min-h-screen bg-[#000d2e] flex items-center justify-center text-center px-4">
        <div>
    ...
    <div className="min-h-screen bg-[#000d2e]">
      ...
      >
        ...
        <div className="max-w-5xl mx-auto">
          {/* Breadcrumb */}
          <div
            className={`flex items-center gap-2 text-sm text-gray-400 mb-8 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            <Link href="/" className="hover:text-white transition-colors">Beranda</Link>
            <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            <span className="hover:text-white transition-colors">Wilayah</span>
            <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          ...
          {/* Title */}
          <div
            className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 transition-all duration-700 delay-100 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            <div
          ...
          <div
            className={`grid grid-cols-1 lg:grid-cols-3 gap-6 transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            {/* Big number card */}
            <div
              className="lg:col-span-2 rounded-2xl p-8 relative overflow-hidden"
              style={{
                background: `linear-gradient(135deg, ${region.color}20, ${region.color}05)`,
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                      {percentage}% dari total semua wilayah
                    </div>
                  </>
                )}
              </div>
          ...
          {/* Dashboard Link */}
          <div
            className={`mt-8 transition-all duration-700 delay-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
          >
            <a
              href={region.gasUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-3 w-full sm:w-auto justify-center px-8 py-5 rounded-2xl font-bold text-base transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] hover:shadow-2xl"
              style={{
                background: `linear-gradient(135deg, ${region.color}, ${region.color}cc)`,
                boxShadow: `0 8px 30px ${region.color}40`,
              }}
            >
            ...
            <p className="text-gray-500 text-xs mt-2 ml-1">
              * Dashboard interaktif akan terbuka di tab baru
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8 border-t border-white/5">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          {prevRegion ? (
            <Link
              href={`/wilayah/${prevRegion.slug}`}
              className="flex items-center gap-3 px-5 py-3 rounded-xl border border-white/10 hover:bg-white/5 hover:border-white/20 transition-all duration-200 group"
            >
          ...
          <Link
            href="/"
            className="flex items-center gap-2 px-5 py-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-200 text-gray-300 hover:text-white text-sm font-medium"
          >
          ...
          {nextRegion ? (
            <Link
              href={`/wilayah/${nextRegion.slug}`}
              className="flex items-center gap-3 px-5 py-3 rounded-xl border border-white/10 hover:bg-white/5 hover:border-white/20 transition-all duration-200 group text-right"
            >
              <div>
